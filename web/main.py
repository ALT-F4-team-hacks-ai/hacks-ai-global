...
@app.route('/addtoglossary')
@login_required
def addtoglossary():
    db_sess = db_session.create_session()
    user = db_sess.query(User).filter(User.id == current_user.id).first()
    user.glossary = json.loads(str({**dict(user.glossary), **dict(terms)}).replace("\'", "\""))
    db_sess.commit()
    return redirect("/")
...
def viewing_profile(id):
    ...
    if classes:
        ...
        return render_template('profzero.html')


@app.route('/glossary/<int:id>', methods=['GET'])
def viewing_glossary(id):
    db_sess = db_session.create_session()
    user = db_sess.query(User).filter(User.id == id).first()
    classes = db_sess.query(Classes).filter(Classes.user_id == id)
    glossary = user.glossary
    print(glossary)
    glossary = json.loads(str(glossary).replace("\'", "\""))
    terms = []
    opr = []
    print(terms)
    for i in glossary:
        print(i)
        terms.append([i, glossary[i]])
